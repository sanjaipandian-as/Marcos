...
*   **Raw Body Preservation**: Signature validation requires the raw request body buffer to compute HMAC checksums. Standard JSON parsing disrupts this buffer formatting. The Express app must preserve the raw body for webhook verification:
    ```typescript
...
```mermaid
erDiagram
    User ||--o{ MeasurementProfile : "has"
    User ||--o{ Appointment : "schedules"
    User ||--o{ StoreVisit : "requests (Customer)"
    User ||--o{ StoreVisit : "assigned (Staff)"
    User ||--o{ Order : "places"
    User ||--o{ SupportTicket : "creates"
    User ||--o{ PointTransaction : "earns/redeems"
    User ||--o{ AuditLog : "triggers"
    User ||--o{ CartItem : "owns"
    User ||--o{ UserCoupon : "redeems"
    
    MeasurementProfile ||--o{ MeasurementHistory : "logs changes"
    ...
    StoreVisit ||--o| VisitReport : "completed with"
    User ||--o{ VisitReport : "written by (Staff)"
    
    Coupon ||--o{ UserCoupon : "tracks utilization"
...
#### `POST /visits`
*   **Access**: Authenticated (Customer)
*   **Body**: Target date, address, and requirements for a mobile tailor visit.

#### `PUT /visits/:id/assign`
*   **Access**: Admin Only
*   **Body**: `{ "assignedStaffId": "staff_uuid" }`
*   **Description**: Sets status to `ASSIGNED` and sends real-time updates to staff.
...
*   **Body**:
    ...
    ```
*   **Description**: Creates an Order. Checks and decrements inventory quantity (updating `stockStatus` dynamically). Instead of generating the PDF and emails synchronously which blocks the HTTP thread, this endpoint schedules background asynchronous jobs on BullMQ and returns immediately.
...
*   Connection middleware decodes the JWT, validates user role, and saves the connection status.
...
| :--- | :--- | :--- | :--- |
| `join:room` | Client -> Server | Server parses room | `{ "room": "admins" }` (Validates RBAC) |
...
```mermaid
graph LR
    HTTP["POST /billing/invoice or Webhook"] -- "Pushes Job" --> Queue["Redis: marcos-jobs Queue"]
    Queue -- "Pops Job" --> Worker["BullMQ Worker Process"]
    
    subgraph Job Processing Tasks
        Worker -- "GENERATE_INVOICE_PDF" --> Task1["Render PDF via PDFKit + Upload to R2 + Save PDF URL"]
        Worker -- "SEND_NOTIFICATION" --> Task2["SendGrid email / Twilio SMS / FCM push alert"]
        Worker -- "CREDIT_REFERRAL_POINTS" --> Task3["Calculate points & update User.pointsBalance"]
    end
```

#### 1. `GENERATE_INVOICE_PDF`
*   **Trigger**: Instantiated upon successful order processing or checkout webhook completion.
*   **Payload**: `{ "orderId": "uuid" }`
*   **Process**:
    1. Fetches item details, pricing, customer details, and measurements.
    2. Uses `pdf.service.ts` (PDFKit) to construct an itemized tax invoice.
    3. Streams output to Cloudflare R2 object storage.
    4. Updates the database `Invoice` table with the resulting R2 URL.
    5. Spawns a downstream `SEND_NOTIFICATION` job to email the PDF link to the client.

#### 2. `SEND_NOTIFICATION`
*   **Trigger**: Triggered directly by app events or downstream from other completed jobs (like PDF creation).
*   **Payload**:
    ```json
    {
      "userId": "uuid",
      ...
      "templates": {
        "email": { "id": "sendgrid-template-id", "data": { "invoiceUrl": "..." } },
        "sms": "Your order has been shipped!",
        "push": { "title": "Order Shipped", "body": "Track your order here..." }
      }
    }
    ```
*   **Process**: Resolves and dispatches the relevant templates via SendGrid, Twilio, or Firebase Cloud Messaging. If an API rate limit or network timeout occurs, BullMQ automatically retries with exponential backoff configuration.

#### 3. `CREDIT_REFERRAL_POINTS`
*   **Trigger**: Fired when an Order status moves from `PENDING` to `PAID`/`COMPLETED`.
*   **Payload**: `{ "orderId": "uuid", "userId": "uuid" }`
*   **Process**:
    1. Validates transaction and fetches the customer profile.
    2. Computes loyalty points earned from the checkout volume (e.g., 1 point per $10 spent).
    3. If the user has a `referredById` and this is their first completed order, awards referral points to both the referrer and referee.
    4. Executes points balance increments inside a database transaction to prevent race conditions.
...
1.  **Environment Variables (`.env`)**:
    ```ini
    PORT=5000
    DATABASE_URL="postgresql://postgres:password@localhost:5432/marcos_db?schema=public"
    REDIS_URL="redis://localhost:6379"
    JWT_ACCESS_SECRET="super-secret-access-key-goes-here"
    JWT_REFRESH_SECRET="super-secret-refresh-key-goes-here"
    CLOUDFLARE_R2_ACCESS_KEY="r2-access-key"
    CLOUDFLARE_R2_SECRET_KEY="r2-secret-key"
    CLOUDFLARE_R2_BUCKET="marcos-uploads"
    CLOUDFLARE_R2_ENDPOINT="https://<accountid>.r2.cloudflarestorage.com"
    SENDGRID_API_KEY="SG.emailkey"
    TWILIO_ACCOUNT_SID="ACtwilio"
    TWILIO_AUTH_TOKEN="authkey"
    TWILIO_PHONE_NUMBER="+123456789"
    ```

2.  **Prisma Commands**:
    *   Generate Prisma Client: `npx prisma generate`
    *   Deploy migrations: `npx prisma migrate dev --name init`
    *   Open Prisma GUI: `npx prisma studio`

### 8.2 Running Tests

The test suite contains Unit and Integration tests powered by `Jest` and `Supertest`.
...
# Run REST API integration tests (Supertest)
npm run test:integration --workspace=apps/backend

# Run real-time WebSocket connection & room tests
npm run test:socket --workspace=apps/backend
```

#### Test Coverage Outline
*   **Unit**: Focuses on tax calculations, invoice item calculations, coupon deduction percentages, and loyalty points floor validation.
*   **Integration**:
    *   Simulates register -> login -> request OTP -> verification -> token refresh loops.
    *   Validates appointment booking conflicts: books same slot simultaneously and asserts second booking gets a `409 Conflict`.
    *   WebSocket tests verify namespace protection, assertion of authorization headers during handshake, and broadcasting checks using a local test Redis server.

### 8.3 Mobile E2E Testing Pointer
*   **Mobile Test Coverage**: Mobile app client flows, local MMKV encryption storage, offline state UI handling, and end-to-end user journeys (e.g., catalog checkout and appointment booking) are tested via **Maestro / Detox** under the mobile project directory. Refer to [mobile/README.md](file:///d:/Zippy/MARCOS/apps/mobile/README.md) for setup and E2E command references.
