...
export default function BestSellersScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (prodRes.success) {
        // Sort by salesCount descending, and fallback to highest price if sales are equal
    ...
    return (
      ...
      >
        <View style={styles.prodImageWrapper}>
          ...
          <View style={[styles.newBadge, { backgroundColor: '#f59e0b' }]}>
            <Text style={[styles.newBadgeText, { fontFamily: fonts.bold }]}>BEST SELLER</Text>
          </View>
        </View>
      ...
      <View style={[styles.header, { borderColor: theme.border }]}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={[styles.headerSubtitle, { fontFamily: fonts.regular, color: theme.text.secondary }]}>
            Explore our most popular customer favorites & designs
          </Text>
        </View>
        <TouchableOpacity 
