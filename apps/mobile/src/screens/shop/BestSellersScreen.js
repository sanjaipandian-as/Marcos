...
export default function BestSellersScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (prodRes.success) {
        // Best sellers defined as premium products with price >= 45,000
        const items = prodRes.data.filter(p => Number(p.price) >= 45000);
        setProducts(items);
      }
    ...
    >
      <View style={styles.prodImageWrapper}>
        ...
        <View style={styles.prodTag}>
          ...
          <Text style={[styles.prodTagText, { fontFamily: fonts.bold }]}>BEST SELLER</Text>
        </View>
      </View>
