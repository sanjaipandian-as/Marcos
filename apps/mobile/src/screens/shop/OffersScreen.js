...
export default function DiscountsScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (prodRes.success) {
        // Discounts: budget-friendly custom tailoring options below 50,000
        const items = prodRes.data.filter(p => Number(p.price) < 50000);
        setProducts(items);
      }
      ...
      console.error('Error loading discounts & promo products:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <View style={styles.prodImageWrapper}>
        ...
        <View style={styles.prodTag}>
          ...
          <Text style={[styles.prodTagText, { fontFamily: fonts.bold }]}>PROMO OFFER</Text>
        </View>
      </View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={[styles.headerSubtitle, { fontFamily: fonts.regular }]}>
            Enjoy dynamic VIP loyalty point matches and referral coupon savings
          </Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={[styles.emptyText, { fontFamily: fonts.medium }]}>
            No active discount campaigns at the moment.
          </Text>
        </View>
      ) : (
        <FlatList
