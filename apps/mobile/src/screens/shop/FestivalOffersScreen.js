...
export default function FestivalOffersScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (prodRes.success) {
        // Festival: select trending products or mid-to-high end items
        const items = prodRes.data.filter(p => p.isTrending || Number(p.price) >= 30000);
        setProducts(items);
      }
      ...
      console.error('Error loading festival offers:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <View style={styles.prodImageWrapper}>
        ...
        <View style={styles.prodTag}>
          ...
          <Text style={[styles.prodTagText, { fontFamily: fonts.bold }]}>FESTIVE 15%</Text>
        </View>
      </View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={[styles.headerSubtitle, { fontFamily: fonts.regular }]}>
            Celebrate the festive seasons with exclusive premium discounts
          </Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={[styles.emptyText, { fontFamily: fonts.medium }]}>
            No active festival offers at the moment.
          </Text>
        </View>
      ) : (
        <FlatList
