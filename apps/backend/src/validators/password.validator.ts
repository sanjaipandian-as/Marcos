export function validatePassword(password: string, context?: { email?: string; fullName?: string }) {
  ...
  if (context?.fullName) {
    const names = context.fullName.toLowerCase().split(/\s+/).filter((n: string) => n.length > 3);
