...
async function seed() {
  ...
  try {
    ...
    const hashedSuperAdmin = await hashPassword('Superadmin123!');
    const hashedAdmin = await hashPassword('Admin123!');
    const hashedStaff = await hashPassword('Staff123!');
    const hashedCustomer = await hashPassword('Customer123!');

    const hashedMarcos = await hashPassword('marcos@zippy');
    ...
    const categoryLehengas = await prisma.category.create({
      data: { name: 'Bridal Lehengas', slug: 'bridal-lehengas', order: 1 },
    });
    const categorySherwanis = await prisma.category.create({
      data: { name: 'Sherwanis', slug: 'sherwanis', order: 2 },
    });
    const categorySuits = await prisma.category.create({
      data: { name: 'Blazers & Suits', slug: 'blazers-suits', order: 3 },
    });
    const categoryAnarkalis = await prisma.category.create({
    ...
    const productLehengaCrimson = await prisma.product.create({
      data: {
    ...
    const productSherwaniGold = await prisma.product.create({
      data: {
    ...
    const productLehengaMint = await prisma.product.create({
      data: {
    ...
    const productAnarkaliMidnight = await prisma.product.create({
      data: {
    ...
    const productSuitCharcoal = await prisma.product.create({
      data: {
    ...
    const measurementJaneSelf = await prisma.measurementProfile.create({
      data: {
        userId: customer1.id,
    ...
    const measurementJohnSelf = await prisma.measurementProfile.create({
      data: {
        userId: customer2.id,
    ...
    const couponFlat1500 = await prisma.coupon.create({
      data: {
        code: 'FESTIVE1500',
    ...
    const completedVisit = await prisma.storeVisit.create({
      data: {
    ...
    const pendingVisit = await prisma.storeVisit.create({
      data: {
