...
export class TicketController {
   ...
   */
  static async sendStatusUpdateEmail(ticketId: string, status: string, subject: string, description: string, userEmail: string, userFullName: string) {
    try {
      let emailSubject = `${env.APP_NAME} Support: Ticket Update - ID: ${ticketId}`;
      let text = '';
      let html = '';

      if (status === 'RESOLVED') {
        emailSubject = `${env.APP_NAME} Support: Ticket Resolved - ID: ${ticketId}`;
        text = `Hello ${userFullName},\n\nOur support team has resolved your ticket.\n\nTicket Details:\n- Ticket ID: ${ticketId}\n- Subject: ${subject}\n- Description: ${description}\n\nThank you for reaching out to ${env.APP_NAME}. Let us know if you need any further assistance.\n\nBest regards,\nThe ${env.APP_NAME} Support Team`;
        html = `
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px;">
            ...
            <p>You can view the resolution details and chat transcript in the ${env.APP_NAME} mobile app.</p>
            <p>Thank you for choosing ${env.APP_NAME}!</p>
            <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;"/>
            <p style="font-size: 11px; color: #64748b; margin-bottom: 0;">This is an automated message from ${env.APP_NAME} Support.</p>
          </div>
        `;
      } else {
        ...
        html = `
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px;">
            ...
            <p>Our team is reviewing and addressing your request. You can track this and chat with support in the ${env.APP_NAME} mobile app.</p>
            <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;"/>
            <p style="font-size: 11px; color: #64748b; margin-bottom: 0;">This is an automated message from ${env.APP_NAME} Support.</p>
          </div>
        `;
      ...
      await EmailService.sendEmail(userEmail, emailSubject, text, html);
    } catch (err) {
  ...
  static async createTicket(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      try {
        const emailSubject = `${env.APP_NAME} Support: Ticket Raised - ID: ${ticket.id}`;
        ...
        const html = `
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px;">
            ...
            <p>You can chat with our team directly regarding this ticket in the support section of the ${env.APP_NAME} app.</p>
            <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;"/>
            <p style="font-size: 11px; color: #64748b; margin-bottom: 0;">This is an automated message from ${env.APP_NAME} Support.</p>
          </div>
        `;
        await EmailService.sendEmail(req.user!.email, emailSubject, text, html);
      } catch (err) {
   ...
   */
  static async getCustomerTicketMessages(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const userId = req.user!.id;
   ...
   */
  static async sendCustomerTicketMessage(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const { text } = req.body;
    const user = req.user!;
  ...
  static async adminUpdateTicket(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (existing.user && existing.user.email) {
        await TicketController.sendStatusUpdateEmail(
          ticket.id,
          status,
          ticket.subject,
          ticket.description,
  ...
  static async adminSendTicketMessage(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (ticket.status === 'OPEN') {
        ...
        const ticketOwner = await prisma.user.findUnique({ where: { id: ticket.userId } });
   ...
   */
  static async uploadTicketImage(req: Request, res: Response, next: NextFunction) {
    try {
      if (!req.file) {
