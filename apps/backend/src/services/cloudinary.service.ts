...
export class CloudinaryService {
   ...
   */
  static async uploadFile(buffer: Buffer, folder: string = env.APP_NAME.toLowerCase(), mimeType: string = 'image/jpeg'): Promise<string> {
    ...
    try {
      ...
      logger.error('Cloudinary upload error:', { metadata: { error: error.message } });
      throw error;
    }
  }
  ...
  static async deleteFile(fileUrl: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        const errText = await response.text();
        logger.error(`Cloudinary deletion failed: ${errText}`);
      }
    } catch (error: any) {
      logger.error('Failed to delete file from Cloudinary:', { metadata: { error: error.message } });
    }
  }
}
