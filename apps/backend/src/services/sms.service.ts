...
const isTwilioConfigured =
  ...
  env.TWILIO_ACCOUNT_SID !== 'ACtwilio' &&
  env.TWILIO_AUTH_TOKEN !== 'authkey' &&
...
if (isTwilioConfigured) {
  try {
    ...
    logger.error('Failed to initialize Twilio client:', { metadata: { error: err.message } });
  }
}
  ...
  static async sendSms(to: string, message: string) {
    if (twilioClient) {
      try {
        ...
        logger.info(`[TWILIO SMS SENT] To: ${to} | MessageSid: ${info.sid}`);
        return { success: true, messageSid: info.sid };
      } catch (err: any) {
        logger.error('Twilio SMS delivery failed. Falling back to SMTP/Logs.', { metadata: { error: err.message } });
      }
    }
    ...
    try {
      ...
      if (user && user.email) {
        ...
        logger.info(`[SMTP FORWARD] Routing SMS to User Email: ${user.email} (Phone: ${to})`);

        const codeMatch = message.match(/\b\d{6}\b/);
        let emailHtml = '';
        let subject = `${env.APP_NAME} Studio Notification`;
            ...
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f9f9f9; padding: 40px 20px; text-align: center;">
              <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); overflow: hidden; border: 1px solid #eaeaea; text-align: left;">
                ...
                <div style="background: linear-gradient(135deg, #111111, #333333); padding: 30px; text-align: center;">
                  <h1 style="color: #ffffff; margin: 0; font-size: 28px; font-weight: 700; letter-spacing: 2px;">${env.APP_NAME}</h1>
                ...
                <div style="padding: 40px 30px;">
                  ...
                  <p style="color: #555555; font-size: 15px; line-height: 1.6; margin: 0 0 30px 0;">
                    A request has been made to log in or verify your identity with **${env.APP_NAME} Studio**. Please use the secure one-time verification code (OTP) below to complete your sign-in.
                  </p>
                  ...
                  <p style="color: #999999; font-size: 13px; line-height: 1.5; margin: 0 0 20px 0;">
                    This code is highly sensitive and is valid for the next **5 minutes**. For your own security, do not share this code with anyone, including the ${env.APP_NAME} support team.
                  </p>
                ...
                <div style="background-color: #f4f6f8; padding: 25px 30px; text-align: center; border-top: 1px solid #eaeaea;">
                  <p style="color: #222222; font-size: 14px; font-weight: 600; margin: 0 0 5px 0;">${env.APP_NAME} Bespoke Tailoring Studio</p>
                  ...
                  <p style="color: #bbbbbb; font-size: 11px; margin: 0; line-height: 1.4;">
                    ...
                    &copy; ${new Date().getFullYear()} ${env.APP_NAME} Studio. All Rights Reserved.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f9f9f9; padding: 40px 20px; text-align: center;">
              <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); overflow: hidden; border: 1px solid #eaeaea; text-align: left;">
                ...
                <div style="background: linear-gradient(135deg, #111111, #333333); padding: 25px; text-align: center;">
                  <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: 2px;">${env.APP_NAME}</h1>
                </div>
                <!-- Content -->
                <div style="padding: 30px 25px;">
                  <h2 style="color: #222222; margin: 0 0 15px 0; font-size: 18px; font-weight: 600;">${env.APP_NAME} Studio Alert</h2>
                ...
                <div style="background-color: #f4f6f8; padding: 20px 25px; text-align: center; border-top: 1px solid #eaeaea;">
                  <p style="color: #222222; font-size: 13px; font-weight: 600; margin: 0 0 5px 0;">${env.APP_NAME} Bespoke Tailoring Studio</p>
                  <p style="color: #bbbbbb; font-size: 11px; margin: 0; line-height: 1.4;">
        ...
        await EmailService.sendEmail(user.email, subject, message, emailHtml);
      ...
      logger.error('Failed to route SMS via SMTP email', { metadata: { error: error.message } });
