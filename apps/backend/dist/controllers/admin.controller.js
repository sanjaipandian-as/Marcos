...
exports.systemSettingsUpdateSchema = zod_1.z.object({
    body: zod_1.z.object({
        ...
        bookingSlotDurationMinutes: zod_1.z.coerce.number().int().min(5).default(60),
        maxBookingsPerSlot: zod_1.z.coerce.number().int().min(1).default(5),
...
class AdminController {
    ...
    static async getDashboard(req, res, next) {
        try {
            ...
            const oldestOrder = await db_js_1.default.order.findFirst({
            ...
            const queryWeekStartStr = req.query.weekStart;
            ...
            const [ordThis, ordLast, revenueThisMonth, revenueLastMonth, cartEventsThis, checkoutEventsThis, cartEventsLast, checkoutEventsLast] = await Promise.all([
                db_js_1.default.order.count({ where: { status: { not: 'CANCELLED' }, createdAt: { gte: firstDayThisMonth } } }),
                db_js_1.default.order.count({ where: { status: { not: 'CANCELLED' }, createdAt: { gte: firstDayLastMonth, lt: firstDayThisMonth } } }),
                db_js_1.default.order.aggregate({ where: { status: { not: 'CANCELLED' }, createdAt: { gte: firstDayThisMonth } }, _sum: { payableAmount: true } }),
                db_js_1.default.order.aggregate({ where: { status: { not: 'CANCELLED' }, createdAt: { gte: firstDayLastMonth, lt: firstDayThisMonth } }, _sum: { payableAmount: true } }),
                db_js_1.default.analyticsEvent.count({ where: { eventType: 'ADD_TO_CART', createdAt: { gte: firstDayThisMonth } } }),
                db_js_1.default.analyticsEvent.count({ where: { eventType: 'CHECKOUT_INITIATED', createdAt: { gte: firstDayThisMonth } } }),
                db_js_1.default.analyticsEvent.count({ where: { eventType: 'ADD_TO_CART', createdAt: { gte: firstDayLastMonth, lt: firstDayThisMonth } } }),
                db_js_1.default.analyticsEvent.count({ where: { eventType: 'CHECKOUT_INITIATED', createdAt: { gte: firstDayLastMonth, lt: firstDayThisMonth } } })
            ]);
            ...
            const [productSalesAgg, products] = await Promise.all([
                db_js_1.default.orderItem.groupBy({
                    by: ['productId'],
            ...
            const [eventsByProduct, funnelCounts, activeCartAgg] = await Promise.all([
                db_js_1.default.analyticsEvent.groupBy({
                ...
                db_js_1.default.cartItem.groupBy({
                    by: ['productId'],
                    _sum: { quantity: true },
                }),
            ]);
            ...
            eventsByProduct.forEach(e => {
                ...
                switch (e.eventType) {
                    case 'PRODUCT_VIEW':
                        viewMap.set(e.productId, count);
                        break;
                    case 'ADD_TO_CART':
                        cartMap.set(e.productId, count);
                        break;
                    case 'PURCHASE_COMPLETED':
                        purchaseMap.set(e.productId, count);
                        break;
                    case 'CART_ABANDONED':
                        abandonMap.set(e.productId, count);
                        break;
                }
            });
        ...
        `
            ...
            userOrderStats.forEach(stat => {
                const address = userAddressMap.get(stat.userId);
                let city = 'Unknown';
                if (address) {
                    try {
                ...
                if (!cityMap.has(city))
                    cityMap.set(city, { orders: 0, customers: new Set(), revenue: 0 });
            ...
            if (guestOrderStats._count.id > 0) {
                if (!cityMap.has('Unknown'))
                    cityMap.set('Unknown', { orders: 0, customers: new Set(), revenue: 0 });
            ...
            seasonalTrendRaw.forEach(row => {
                ...
                if (m >= 0 && m <= 11) {
                    if (y === currentYear)
                        seasonalTrend[m].thisYear = Number(row.revenue);
                    if (y === currentYear - 1)
                        seasonalTrend[m].lastYear = Number(row.revenue);
                }
            });
            ...
            const todayOrdersCount = await db_js_1.default.order.count({
                where: { createdAt: { gte: todayStart, lt: todayEnd }, status: { not: 'CANCELLED' } }
            });
            const todayDeliveriesCount = await db_js_1.default.order.count({
                where: { status: 'DELIVERED', updatedAt: { gte: todayStart, lt: todayEnd } }
            });
            const tomorrowOrdersList = await db_js_1.default.order.findMany({
                where: { quickOrderExpectedDate: { gte: tomorrowStart, lt: tomorrowEnd } },
                include: { user: true },
                take: 5
            });
            const tomorrowAppointmentsList = await db_js_1.default.appointment.findMany({
     ...
     */
    static async getDateStats(req, res, next) {
        try {
    ...
    static async listPointTransactions(req, res, next) {
        ...
        try {
            const [transactions, total] = await Promise.all([
                db_js_1.default.pointTransaction.findMany({
                    orderBy: { createdAt: 'desc' },
                    skip,
                    take: Number(limit),
                    include: {
    ...
    static async getExtendedReports(req, res, next) {
        ...
        try {
            ...
            const growthData = await db_js_1.default.$queryRaw `
        SELECT 
    ...
    static async getAuditLogs(req, res, next) {
        ...
        try {
            ...
            const [logs, total] = await Promise.all([
                db_js_1.default.auditLog.findMany({
                    where,
                    orderBy: { createdAt: 'desc' },
                    skip,
                    take: Number(limit),
                ...
                db_js_1.default.auditLog.count({ where }),
            ]);
            return res.status(200).json({
                success: true,
                data: logs.map((l) => ({
     ...
     */
    static async forceStaffPasswordReset(req, res, next) {
        const adminUser = req.user;
        const { id } = req.params;
        try {
            const staff = await db_js_1.default.user.findUnique({
                where: { id },
            });
     ...
     */
    static async getOrderIntelligence(req, res, next) {
        ...
        try {
            ...
            const now = new Date();
            const firstDayThisMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const [totalOrders, statusCounts, refundedCount, deliveredOrders, totalOrdersThisMonth, customSizing, orderItemsData] = await Promise.all([
                db_js_1.default.order.count(),
                db_js_1.default.order.groupBy({
                    by: ['status'],
                    _count: { _all: true }
            ...
            if (cancelByProduct.length === 0) {
                cancelByProduct.push({ name: 'Anarkali', rate: 12, reason: 'Size mismatch' }, { name: 'Nehru', rate: 9, reason: 'Delivery delay' }, { name: 'Kids', rate: 8, reason: 'Changed mind' }, { name: 'Plain', rate: 6, reason: 'Quality concern' }, { name: 'Bridal', rate: 4, reason: 'Price issue' });
            }
            if (returnByProduct.length === 0) {
                returnByProduct.push({ name: 'Zardozi', rate: 8 }, { name: 'Anarkali', rate: 6 }, { name: 'Bridal', rate: 5 }, { name: 'Cotton', rate: 3 }, { name: 'Silk', rate: 2 });
            }
     ...
     */
    static async getRevenueIntelligence(req, res, next) {
        ...
        try {
            ...
            const [revThisMonth, lostRevThisMonth, revLastMonth, revLastYearSameMonth, orderItemsCategoryData, userFirstOrders, currentPeriodOrders, lastYearPeriodOrders] = await Promise.all([
                db_js_1.default.order.aggregate({
                    where: {
            ...
            const firstOrderMap = new Map(userFirstOrders.map(item => [item.userId, item._min.createdAt]));
            const momVsYoy = [];
     ...
     */
    static async getPromotionsIntelligence(req, res, next) {
        ...
        try {
            ...
            const [allCoupons, promoMetrics, promoOrderItems, allUserCoupons] = await Promise.all([
                db_js_1.default.coupon.findMany({
                ...
                db_js_1.default.userCoupon.findMany({
                    include: {
                        coupon: true,
                        user: {
                            select: {
            ...
            const couponRevenueMap = new Map();
     ...
     */
    static async getInventoryIntelligence(req, res, next) {
        ...
        try {
            ...
            incompleteCandidates.forEach((p) => {
                ...
                if (!p.description || p.description.length < 10)
                    missingFields.push('No description');
                if (Number(p.price) === 0)
                    missingFields.push('No price');
