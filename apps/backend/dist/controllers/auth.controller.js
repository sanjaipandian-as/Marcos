...
exports.redeemPointsSchema = zod_1.z.object({
    body: zod_1.z.object({
        voucherPlanId: zod_1.z.string().uuid().optional(),
        pointsToRedeem: zod_1.z.coerce.number().int().optional(),
    }),
});
...
class AuthController {
    ...
    static async login(req, res, next) {
        ...
        try {
            ...
            await redis_js_1.default.del(`failed_login:${email}`);
            await redis_js_1.default.del(`failed_login_ip:${req.ip}`);
            const payload = {
                id: user.id,
                email: user.email,
                role: user.role,
    ...
    static async sendOtp(req, res, next) {
        ...
        try {
            ...
            const sendCooldown = await redis_js_1.default.get(`cooldown:send:otp:${identifier}`);
            if (sendCooldown) {
                return res.status(429).json({
                    success: false,
            ...
            const count = await redis_js_1.default.incr(hourKey);
            if (count === 1) {
                await redis_js_1.default.expire(hourKey, 3600); // 1 hour TTL
            }
            if (count > 5) {
                // Block user for 24 hours if they trigger excessive OTPs
                await redis_js_1.default.set(`cooldown:otp:${identifier}`, 'blocked', 'EX', 86400);
                await redis_js_1.default.del(hourKey);
                await (0, audit_js_1.createAuditLog)({
            ...
            if (phoneNumber) {
                sms_service_js_1.default.sendSms(identifier, `Your MARCOS verification code is ${code}. Valid for 5 minutes.`).catch(err => {
                    logger_js_1.default.error('Failed to send SMS OTP in background', { metadata: { error: err.message } });
                });
            }
            if (email) {
                const emailHtml = `
        ...
        `;
                email_service_js_1.default.sendEmail(email, 'Your MARCOS Verification Code', `Your MARCOS OTP is: ${code}`, emailHtml).catch(err => {
                    logger_js_1.default.error('Failed to send Email OTP in background', { metadata: { error: err.message } });
                });
            }
            // Set 60-second request cooldown on success
            await redis_js_1.default.set(`cooldown:send:otp:${identifier}`, 'active', 'EX', 60);
    ...
    static async verifyOtp(req, res, next) {
        ...
        try {
            ...
            const storedHash = await redis_js_1.default.get(`otp:${identifier}`);
            ...
            if (!storedHash || storedHash !== hashedIncoming) {
                ...
                if (attempts === 1)
                    await redis_js_1.default.expire(failKey, 900); // 15 mins tracking
                if (attempts >= 3) {
                    // Block identifier for 15 minutes
                    await redis_js_1.default.set(`cooldown:otp:${identifier}`, 'blocked', 'EX', 900);
            ...
            await redis_js_1.default.del(`otp:${identifier}`);
            await redis_js_1.default.del(`otp_fail:${identifier}`);
            ...
            if (phoneNumber) {
                ...
                const raw = clean.startsWith('+91') ? clean.substring(3) : (clean.startsWith('91') ? clean.substring(2) : clean);
                user = await db_js_1.default.user.findFirst({
                    where: {
    ...
    static async logout(req, res, next) {
        ...
        try {
            if (token) {
                await auth_service_js_1.default.logoutRefreshToken(token);
            }
            if (authHeader && authHeader.startsWith('Bearer ')) {
                const accessToken = authHeader.split(' ')[1];
                await auth_service_js_1.default.blacklistAccessToken(accessToken);
            }
    ...
    static async forgotPassword(req, res, next) {
      ...
      `;
            email_service_js_1.default.sendEmail(email, 'Reset Your MARCOS Password', `Your password reset code is: ${code}`, emailHtml).catch(err => {
                logger_js_1.default.error('Failed to send Reset Password email in background', { metadata: { error: err.message } });
            });
            return res.status(200).json({ success: true, message: 'If that email exists, a reset code has been sent.' });
        }
        catch (error) {
    ...
    static async redeemPoints(req, res, next) {
        ...
        try {
            const result = await db_js_1.default.$transaction(async (tx) => {
                ...
                const code = `REDEEM-${pointsRequired}-${crypto_1.default.randomBytes(3).toString('hex').toUpperCase()}`;
